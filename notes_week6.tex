...
As discussed, there are situations where we would like to distinguish between identical
(or equal) types and \emph{equivalent} types, written $A\simeq B$.
...
Since we are in a proof-relevant setting, the information that $A\simeq B$ consists of five
things:
\begin{itemize}
...
\begin{enumerate}
 ...
 \item If $f: A\to B$ is an equivalence, then there is $f^{-1}: B\to A$ (given by the quasi-inverse
 of $f$) that is also an equivalence.
...
\begin{proof}
 As noted in Section 2, it suffices to produce a quasi-inverse for $f$. We need to construct three objects:
 \begin{enumerate}
 ...
 \begin{enumerate}
  ...
  and
  $$\ap2_f: \Id{}(x,x') \to \Id{}(y,y') \to \Id{}(f x y, f x' y')$$ %Should we fill this out more?
  ...
  By path induction, we need to show that
